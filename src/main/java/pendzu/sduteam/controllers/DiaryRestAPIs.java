...
public class DiaryRestAPIs {
  ...
  public ResponseEntity<?> searchDiaryByTitleAndUserId(@PageableDefault(value = 2) Pageable pageable,
                                                       @RequestBody SearchDiaryByTitleAndUserId searchDiaryByTitleAndUserId) {
    List<Diary> diaries;
    if (searchDiaryByTitleAndUserId.getTitle().equals("")) {
      diaries = (List<Diary>) diaryService.findAllByUserIdAndStatus(pageable, searchDiaryByTitleAndUserId.getId(), allPostAvailable);
      if (diaries.isEmpty()) {
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
      }
      return new ResponseEntity<>(diaries, HttpStatus.OK);
  ...
  @PostMapping("/diary/search-by-titleAndStatus")
  public ResponseEntity<?> searchDiaryByTitleAndStatus(@RequestBody String title) {
